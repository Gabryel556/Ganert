...
/* DESIGN DA FOTO DE PERFIL COM MOLDURA TECH FRIA */
...
/* NOVA MOLDURA: CICLO DE CORES FRIAS (Branco, Ciano, Azul Tech) */
.profile-frame::before {
  content: '';
  position: absolute;
  top: -4px; left: -4px; right: -4px; bottom: -4px;
  
  /* Paleta: Branco -> Ciano Água -> Azul Elétrico -> Branco (seamless loop) */
  ...
  /* Tamanho expandido para a animação de deslize */
  background-size: 400% 400%;
  border-radius: 22px;
  z-index: 1;
  
  /* Animação lenta e suave (10s) para o efeito de "ciclo" */
  animation: frameColorCycle 10s ease infinite;
}

/* Brilho (Glow) adaptado para as novas cores frias */
.profile-glow {
  position: absolute;
  ...
  border-radius: 20px;
  filter: blur(25px);
  opacity: 0.15; /* Sutil para não ofuscar */
  z-index: 0;
}

/* Animação que desliza o gradiente */
